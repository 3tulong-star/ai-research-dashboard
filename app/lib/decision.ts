export type DecisionInput = {
  ...
  revenueGrowth: number; valuationPercentile: number | null;
...
export function decide(s: DecisionInput) {
  ...
  if (!s.dataComplete) failures.push("关键数据不完整，否决");
  if (!Number.isFinite(s.valuationPercentile)) failures.push("估值历史数据缺失，否决");
  ...
  const valuation = Number.isFinite(s.valuationPercentile) ? 100 - Number(s.valuationPercentile) : 0;
